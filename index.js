...
import { BASE_PATH, p } from './src/config.js'
...
const server = await createServer({
  ...
  idleTimeout: 0,
  // WebSocket upgrade route — client connects to BASE_PATH/ws
  ...
  routes: {
    ...
    // Service worker at BASE_PATH scope so it can receive push events for all app pages
    [p('/sw.js')]: () => new Response(
      Bun.file(import.meta.dir + '/pages/public/sw.js'),
      { headers: { 'Content-Type': 'application/javascript; charset=utf-8', 'Service-Worker-Allowed': `${BASE_PATH}/`, 'Cache-Control': 'no-cache, no-store' } }
    ),
    // Dynamic PWA manifest — start_url and icon.src must reflect BASE_PATH
    [p('/manifest.json')]: () => new Response(
      JSON.stringify({
        ...
        theme_color: '#141517',
        icons: [
          { src: `${BASE_PATH}/icon.png`, sizes: '300x300', type: 'image/png', purpose: 'any maskable' }
        ]
      }),
...
logger.info('server.ready', { port: server.port, dev, basePath: BASE_PATH })
